...
public class HomeController {
	...
	public ModelAndView findContributor(String contributor_name, String contributor_state) {
		...
		String url = "https://api.followthemoney.org/?d-et=2&d-ad-cty=DETROIT&d-ad-st=" + contributor_state + "&APIKey=" + ftmKey + "&mode=json";
		try {
			respEnt = rt
					.exchange(url, HttpMethod.GET, httpEnt, DonorValue.class);
	...
	public ModelAndView findContributor(String contributor_name, String contributor_state) {
		...
		try {
			respEnt = rt
					.exchange("http://api.open.fec.gov/v1/schedules/schedule_a/?api_key=" + fecKey
							+ "&contributor_name=" + contributor_name + "&contributor_state=" + contributor_state, HttpMethod.GET, httpEnt, DonorValue.class);
			return new ModelAndView("result-page", "result", respEnt.getBody().getResults());
		} catch (RestClientException | IndexOutOfBoundsException e) {
			ModelAndView mv = new ModelAndView("index");
			mv.addObject("badSearch", "No results");
